...
import { CalendarType, DayjsInstance, JalaliDayjs } from "./type";

export const calendarJalali = (o: unknown, Dayjs: any, dayjs: any) => {
  const proto = Dayjs.prototype;
  const U = proto.$utils();
  const $isJalali = (v: { $C?: string }): boolean => v.$C === "jalali";
  ...
  const wrapperOfTruth = (action: Function) =>
    function (this: DayjsInstance, ...args: any[]) {
  ...
  dayjs.$C = "gregory";
  dayjs.$fdow = 6; // First day of week (Saturday)

  dayjs.calendar = function (calendar: CalendarType) {
    dayjs.$C = calendar;
    return dayjs;
  };

  proto.calendar = function (calendar: CalendarType) {
  ...
  const wrapper = function (date: Date, instance: JalaliDayjs): JalaliDayjs {
    return dayjs(date, {
      locale: instance.$L,
      utc: instance.$u,
      calendar: instance.$C,
  ...
  proto.startOf = function (units: UnitType, startOf?: boolean) {
    // startOf -> endOf
    if (!$isJalali(this)) {
    ...
    const self = this as JalaliDayjs;
    const instanceFactory = (d: number, m: number, y = self.$jy) => {
      const [gy, gm, gd] = jdate.G(y, m + 1, d);
      const ins = wrapper(new Date(gy, gm - 1, gd), self);
      return (isStartOf ? ins : ins.endOf(C.D)).$set("hour", 1);
    };
  ...
  proto.$set = function (units: UnitType, int: number) {
    if (!$isJalali(this)) {
  ...
  proto.add = function (number: number, units: UnitType) {
    ...
    const unit =
      units && (units.length === 1 || units === "ms")
        ? units
        : $prettyUnit(units);
    const instanceFactory = (u: string, n: string) => {
  ...
  proto.format = function (formatStr: string, localeObject: any) {
    if (!$isJalali(this)) {
  ...
  proto.diff = function (input: number, units: UnitType, float: boolean) {
    if (!$isJalali(this)) {
  ...
  proto.$g = function (input: number, get: string, set: string) {
    if ($isUndefined(input)) return this[get];
    return this.set(set, input);
  };

  proto.year = function (input: number) {
    if (!$isJalali(this)) {
  ...
  proto.month = function (input: number) {
    if (!$isJalali(this)) {
  ...
  proto.date = function (input: number) {
    if (!$isJalali(this)) {
